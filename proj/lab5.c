  ...
  for(int j = 0 ; j < no_rectangles; j++) {
      for (int i = 0; i < no_rectangles; i++) {
          ...
          if(mode == 0x105){
              ...
              color |= ( ((first&(BIT(7) | BIT(6) | BIT(5)) >>5) + i * step) % (1 << vmi_p.RedMaskSize)) << 5;

              color |= (((first&(BIT(4) | BIT(3) | BIT(2))>>2) + i * step) % (1 << vmi_p.GreenMaskSize)) << 2;

              color |= (first + (j + i) * step) % (1 << vmi_p.BlueMaskSize);
          }

          draw_rectangle(i*width,j*height, height,width,color, video_mem, vmi_p);
      }
  }
...
int(video_test_xpm)(xpm_map_t xpm, uint16_t x, uint16_t y) {
    ...
    uint8_t *img_it;
    img_it = img.bytes;
    uint64_t video_it = (uint64_t) video_mem + ((x + (y*1024)));

    for (int i = 0; i < img.height; i++){
        for(int j = 0; j < img.width; j++){
            memset((void*)video_it, img_it[(j + i*img.width)], 1);
